...
expresion_aritmetica
    // Una expresión es una secuencia de términos sumados o restados
    :   termino
    |   invocacion_funcion
    |   TOI '(' expresion_aritmetica ')'
    |   expresion_aritmetica '+' termino
    |   expresion_aritmetica '-' termino
    |   expresion_aritmetica '+' error { Logger.logError(cursor.getCurrentLine(), "Error sintáctico: Falta operando derecho después de '+'."); }
    |   expresion_aritmetica '-' error { Logger.logError(cursor.getCurrentLine(), "Error sintáctico: Falta operando derecho después de '-'."); }
    |  '+' error { Logger.logError(cursor.getCurrentLine(), "Error sintáctico: Falta operando izquierdo para el operador '+'."); }
    |  '-' error { Logger.logError(cursor.getCurrentLine(), "Error sintáctico: Falta operando izquierdo para el operador '+'."); }
    ;
invocacion_funcion
    :   ID '(' lista_param_reales ')'
termino
    // Un término es una secuencia de factores multiplicados o divididos
    :   factor
    |   '-' factor %prec UMINUS          // Menos unario (alta precedencia)
    |   termino '*' factor
    |   termino '/' factor

    // --- REGLAS DE ERROR CORREGIDAS ---
    // Error: Falta operando DERECHO
    |   termino '*' error { Logger.logError(cursor.getCurrentLine(), "Error sintáctico: Falta operando derecho después de '*'."); }
    |   termino '/' error { Logger.logError(cursor.getCurrentLine(), "Error sintáctico: Falta operando derecho después de '/'."); }
...
condicion_error
    :   expresion_aritmetica factor   {Logger.logError(cursor.getCurrentLine(), "Falta de simbolo comparador en condicion");}
    |   expresion_aritmetica simbolo_comparador   {Logger.logError(cursor.getCurrentLine(), "Falta de argumento derecho en condicion");}
    |   simbolo_comparador factor    {Logger.logError(cursor.getCurrentLine(), "Falta de argumento izquierdo en condicion");}
    ;
